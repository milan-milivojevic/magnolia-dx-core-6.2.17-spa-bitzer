...
function ImageTeaser ({
  ...
  useEffect(() => {
    fetch(`${apiBase}${restPath}${nodeName}/Config-Pages/Teasers-Config/imageTeaserComponents/@nodes`)
  ...
  return (    
    ...
    >
      <div className={`imageTeaserComponent flex`} style={imageTeaserComponentStyles}>
        <img className="image" style={{objectPosition: imagePosition || configProps?.imagePosition || "top left"}} ref={dimensionsRef} src={image['@link']} alt="" />
        ...
        > {headline &&    
            <div className='headlineWrapper' style={headlineWrapperStyles}>
              <HeadlineLevel className={`headline ${headlineBackground || configProps?.headlineBackground}`} style={headlineStyles}>
                ...
                </span><span dangerouslySetInnerHTML={{ __html:headline || null }}></span>
              </HeadlineLevel>
            </div>
          }           
            ...
            {(linkIcons || linkLabel) &&
              <div className='linkComponent flex' style={linkComponentStyles}>
                <a className='link' href={href} target={linkLocation || "_blank"} rel="noreferrer" style={linkStyles}>
                  {linkLabel ? linkLabel : ""}
                  {linkIcons === "BsChevronRight" ? <BsChevronRight /> : linkIcons === "ArrowsIcon" ? <ArrowsIcon /> : linkIcons === "TfiDownload" ? <DownloadIcon /> : ""}
                </a>
              </div>
            }
          </div>
