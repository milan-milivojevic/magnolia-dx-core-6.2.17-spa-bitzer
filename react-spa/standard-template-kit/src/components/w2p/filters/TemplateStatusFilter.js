...
export default function TemplateStatusFilter({ onUpdateSelectedTemlateStatus, selectedTemlateStatus }) {

  const [parents, setParents] = useState([]);
  const [selectedOption, setSelectedOption] = useState(null);

  useEffect(() => {
    fetch("https://www.bitzer-marketing-portal.com/wp/rest/search-filters/templates")
      ...
      .then((data) => {
        ...
        const correspondingSelected = transformedParents.find(parent => parent.value === selectedTemlateStatus);
  ...
  const handleSelectChange = (e) => {
    ...
    onUpdateSelectedTemlateStatus(selectedValue);
  };

  return (
    <div className="searchFilter templateStatus">
      <FormControl fullWidth variant="outlined">
        <InputLabel id="details-label">Template Status</InputLabel>
        <Select
